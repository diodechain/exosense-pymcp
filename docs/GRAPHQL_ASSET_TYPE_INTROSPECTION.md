...
- **assetType on Asset**: The API exposes `assetType { name }` on each asset. We request it via `includeAssetType: true` and use it for matching and display alongside template.
...
- **groups_by_asset_type** uses the same fuzzy find (name + template + assetType), then aggregates by customer (first level below root).
